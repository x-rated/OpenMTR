...
static const QStringList COLUMNS = {
    "Hop",  "ASN",     "Hostname", "IP",      "Loss %",  "Sent",
    "Recv", "Best ms", "Avrg ms",  "Wrst ms", "Last ms", "Jttr ms"};
...
static QString ovAccentBlend(const QColor &fg, const QColor &base,
                             double alpha) {
...
static QString formatDuration(qint64 ms) {
  ...
  int h = static_cast<int>(secs / 3600),
      m = static_cast<int>((secs % 3600) / 60), s = static_cast<int>(secs % 60);
  return h > 0 ? QString("%1:%2:%3")
                     .arg(h)
                     .arg(m, 2, 10, QChar('0'))
                     .arg(s, 2, 10, QChar('0'))
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
  ...
  connect(qApp, &QApplication::focusChanged, this, [this](QWidget *, QWidget *now) {
    bool isInteractive =
        now && (now->inherits("QPushButton") || now->inherits("QCheckBox") ||
                now->inherits("QLineEdit") || now->inherits("QAbstractSpinBox"));
        if (!isInteractive) {
          m_keyboardFocus = false;
          hideFocusRing();
          for (auto *w : std::initializer_list<QWidget *>{
                   m_startStopBtn, m_copyBtn, m_exportBtn, m_themeBtn,
                   m_infoBtn, m_ipv6Check, m_targetEdit, m_pingSizeBox}) {
  ...
  tuneEditShortcuts(
      m_targetEdit); // the app opens with the target field focused
}
...
unsigned MainWindow::getPingSize() const noexcept {
  return static_cast<unsigned>(m_pingSizeBox->value());
}
...
void MainWindow::setupUi() {
  ...
  m_targetEdit->setFixedWidth(395);
  m_targetEdit->setFixedHeight(32);
  ...
  connect(m_targetEdit, &QLineEdit::textChanged, this,
          [updateClear](const QString &) { updateClear(); });
  ...
  m_pingSizeBox->setFixedWidth(54);
  m_pingSizeBox->setFixedHeight(32);
  ...
  m_themeBtn->setFixedWidth(36);
  m_themeBtn->setFixedHeight(32);
  ...
  m_infoBtn->setFixedWidth(36);
  m_infoBtn->setFixedHeight(32);
  ...
  connect(m_infoBtn, &QPushButton::clicked, this, [this]() {
    MicaDialog::show(
        ...
        QString("Version %1 (%2) · Qt %3\n\n"
                "Continuously traces the route to a host and shows per-hop "
                "latency and packet loss statistics in real time.\n\n"
                "© slamb.eu · GPL-2.0 license")
            .arg(OPENMTR_VERSION)
            .arg(QSysInfo::buildCpuArchitecture().toUpper().replace("X86_64",
                                                                    "AMD64"))
            .arg(QT_VERSION_STR),
        m_darkMode, "https://github.com/x-rated/OpenMTR", "GitHub");
  });
  ...
  connect(&m_cellTipTimer, &QTimer::timeout, this, [this]() {
    if (!m_cellTipHover.isValid())
      return;
    const QString tip = cellTooltipText(m_cellTipHover);
    if (tip.isEmpty())
      return;
    if (!m_cellTip)
      m_cellTip = new Win11Tooltip();
    m_tipIndex = m_cellTipHover;
    m_tipText = tip;
    m_cellTip->popup(tip, QCursor::pos(), m_darkMode,
                     window()->frameGeometry());
  });

  m_focusRing = new FocusRing(this);
  m_focusRing->setRingColor(m_darkMode ? QColor(255, 255, 255)
                                       : QColor(0, 0, 0, 230));
  m_focusRing->hide();
  ...
  m_table->horizontalHeader()->setSectionResizeMode(ColHostname,
                                                    QHeaderView::Stretch);
  m_table->horizontalHeader()->setSectionResizeMode(ColIp,
                                                    QHeaderView::Stretch);
  ...
  for (int c :
       {ColAsn, ColSent, ColRecv, ColBest, ColAvrg, ColWrst, ColLast, ColJttr})
  ...
  for (int c = 0; c < COLUMNS.size(); ++c) {
    ...
    if (name.endsWith(QStringLiteral(" %"))) {
      unit = QStringLiteral("%");
      name.chop(2);
    } else if (name.endsWith(QStringLiteral(" ms"))) {
...
void MainWindow::applyDarkTheme() {
  ...
  if (m_themeBtn) {
#ifdef Q_OS_WIN
    m_themeBtn->setText(QString(QChar(0xE793)));
#else
    m_themeBtn->setText(QString(QChar(0x263C)));
#endif
  }
  QString sheet = R"(
...
QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { height: 0px; }
  ...
  //   bottom    = ControlStrokeColorOnAccentSecondary #23000000 (35/255 black,
  //   dark theme)
  sheet.replace("$BRDSIDE", ovAccentBlend(QColor(255, 255, 255), m_accent,
                                          20.0 / 255.0)); // top/side hairline
  sheet.replace("$BRDBOT", ovAccentBlend(QColor(0, 0, 0), m_accent,
                                         35.0 / 255.0)); // bottom elevation
  sheet.replace("$BRDPRESS",
                ovAccentBlend(m_accent, QColor(32, 32, 32),
                              0.80)); // pressed fill twin, over dark Mica
  ...
  if (m_tableHeader)
    m_tableHeader->setDark(true);
  if (m_tableScroll)
    m_tableScroll->setDark(true);
  if (m_ipv6Check)
    m_ipv6Check->setDark(true);
  if (m_focusRing)
    m_focusRing->setRingColor(QColor(255, 255, 255));
  if (m_titleBar)
    m_titleBar->setDark(true);
...
QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { height: 0px; }
  ...
  // edge, so the colour is unchanged — only the alpha is baked out.
  //   top/sides = ControlStrokeColorOnAccentDefault   #14FFFFFF (20/255 white)
  //   bottom    = ControlStrokeColorOnAccentSecondary #66000000 (102/255 black,
  //   light theme)
  sheet.replace("$BRDSIDE", ovAccentBlend(QColor(255, 255, 255), m_accent,
                                          20.0 / 255.0)); // top/side hairline
  sheet.replace("$BRDBOT", ovAccentBlend(QColor(0, 0, 0), m_accent,
                                         102.0 / 255.0)); // bottom elevation
  sheet.replace("$BRDPRESS",
                ovAccentBlend(m_accent, QColor(243, 243, 243),
                              0.80)); // pressed fill twin, over light Mica
  ...
  if (m_tableHeader)
    m_tableHeader->setDark(false);
  if (m_tableScroll)
    m_tableScroll->setDark(false);
  if (m_ipv6Check)
    m_ipv6Check->setDark(false);
  if (m_focusRing)
    m_focusRing->setRingColor(QColor(0, 0, 0, 230));
  if (m_titleBar)
    m_titleBar->setDark(false);
...
void MainWindow::updateInputStyle(QWidget *input, InputAccentBar *accent,
                                  bool active, bool keyboardFocused) {
  if (!input)
    return;
  ...
  if (active) {
    ...
    if (accent) {
      ...
      accent->setGeometry(0, input->height() - thickness, input->width(),
                          thickness);
...
void MainWindow::showFocusRing(QWidget *w) {
  if (!m_focusRing || !isFocusRingTarget(w))
    return;
...
static UINT autoHideTaskbarEdge(HWND hwnd) {
  ...
  if (edgeHasBar(ABE_BOTTOM))
    return ABE_BOTTOM;
  if (edgeHasBar(ABE_TOP))
    return ABE_TOP;
  if (edgeHasBar(ABE_LEFT))
    return ABE_LEFT;
  if (edgeHasBar(ABE_RIGHT))
    return ABE_RIGHT;
  return static_cast<UINT>(-1);
}
#endif
...
bool MainWindow::nativeEvent(const QByteArray &eventType, void *message,
  ...
  if (eventType == "windows_generic_MSG") {
    ...
    // pull the client rect in by 1px on an auto-hide-taskbar edge so it stays
    // reachable.
    case WM_NCCALCSIZE: {
      if (msg->wParam != TRUE)
        break;
    ...
    // WM_NCACTIVATE: keep the custom title bar looking active even when
    // unfocused.
    case WM_NCACTIVATE:
      *result = ::DefWindowProcW(msg->hwnd, WM_NCACTIVATE, msg->wParam, -1);
      return true;

    // WM_NCHITTEST: classify the cursor for a frameless window — caption
    // buttons, the resize borders, the draggable caption, or plain client area.
    case WM_NCHITTEST: {
      const QPoint ptScreen(GET_X_LPARAM(msg->lParam),
      ...
      if (m_titleBar) {
        ...
        if (!closeR.isNull() && pos.y() >= closeR.top() &&
            pos.y() <= closeR.bottom() && pos.x() >= closeR.left()) {
    ...
    case WM_NCLBUTTONDOWN:
      ...
      if (msg->wParam == HTCLOSEBUTTON && m_titleBar &&
          m_titleBar->closeBtn()) {
    ...
    // WM_SETTINGCHANGE: a system light/dark switch — re-apply the matching
    // theme.
    case WM_SETTINGCHANGE:
      if (msg->lParam && std::wstring(reinterpret_cast<LPCWSTR>(msg->lParam)) ==
                             L"ImmersiveColorSet") {
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event) {
  ...
  if (event->type() == QEvent::KeyPress) {
    ...
    m_keyboardFocus =
        (ke->key() == Qt::Key_Tab || ke->key() == Qt::Key_Backtab);
  }
  ...
  if (m_table && obj == m_table->viewport()) {
    if (event->type() == QEvent::MouseMove) {
      auto *me = static_cast<QMouseEvent *>(event);
      if (m_tableDelegate) {
        const bool scrollDragging =
            m_tableScroll && m_tableScroll->isDragging();
        const bool inScrollZone =
            m_tableScroll && me->pos().x() >= m_table->viewport()->width() -
                                                  m_tableScroll->width();
        const int row = (scrollDragging || inScrollZone)
                            ? -1
                            : m_table->indexAt(me->pos()).row();
      ...
      QTimer::singleShot(0, this, [this]() {
        if (m_toolbar)
          m_toolbar->setFocus(Qt::NoFocusReason);
      });
    }
  }
  ...
  if (event->type() == QEvent::FocusIn || event->type() == QEvent::FocusOut) {
    ...
    if (w) {
      ...
      if (event->type() == QEvent::FocusIn) {
        if (keyboardFocus && isFocusRingTarget(w))
          showFocusRing(w);
        else
          hideFocusRing();
      ...
      if (w == m_targetEdit || w == m_pingSizeBox)
        input = w;
      else if (w->parentWidget() == m_pingSizeBox)
        input = m_pingSizeBox;
      if (input) {
        const bool silentFocus =
            (event->type() == QEvent::FocusIn) && (reason == Qt::NoFocusReason);
        bool inputActive = (event->type() == QEvent::FocusIn) && !silentFocus;
        updateInputStyle(input,
                         input->findChild<InputAccentBar *>("inputAccent"),
                         inputActive, focused);
      }
    }
  }
  ...
  if (event->type() == QEvent::KeyPress) {
    ...
    if (btn && btn->isEnabled()) {
      ...
      if (keyEvent->key() == Qt::Key_Return ||
          keyEvent->key() == Qt::Key_Enter) {
        if (btn == m_startStopBtn)
          onStartStop();
        else if (btn == m_copyBtn)
          onCopy();
        else if (btn == m_exportBtn)
          onExport();
        else if (btn == m_themeBtn)
          onToggleTheme();
        else if (btn == m_infoBtn)
          m_infoBtn->click();
        return true;
      }
    }
...
void MainWindow::mousePressEvent(QMouseEvent *event) {
  ...
  bool isInteractive =
      clicked && (clicked->inherits("QPushButton") ||
                  clicked->inherits("QCheckBox") ||
                  clicked->inherits("QLineEdit") ||
                  clicked->inherits("QAbstractSpinBox"));
  if (!isInteractive) {
    m_keyboardFocus = false;
    hideFocusRing();
    if (auto *fw = QApplication::focusWidget())
...
void MainWindow::showIconTooltip(QPushButton *btn, const QString &text) {
  if (!btn || !btn->isVisible() || text.isEmpty())
    return;
  sharedCaptionTooltip()->popup(text, QCursor::pos(), m_darkMode,
                                window()->frameGeometry());
}
...
QString MainWindow::lookupASN(const QString &ip, bool ipv6) {
  ...
  DNS_STATUS status =
      DnsQuery_W(query.toStdWString().c_str(), DNS_TYPE_TEXT,
  ...
  if (m_tracing) {
    ...
    connect(pollTimer, &QTimer::timeout, this,
            [self, pollTimer, oldNet]() mutable {
              ...
              if (oldNet && !oldNet->isDone())
                return;
              pollTimer->stop();
              pollTimer->deleteLater();
              oldNet.reset();
              if (!self->m_tracing)
                self->m_startStopBtn->setEnabled(
                    !self->m_targetEdit->text().trimmed().isEmpty());
            });
    pollTimer->start();
    m_targetEdit->setFocus();
  } else {
    ...
    if (m_titleBar)
      m_titleBar->setSubtitle(QString());
    ...
    QTimer::singleShot(
        0, this, [self, provider, target, wantFamily, darkMode]() {
          if (!self)
            return;
          std::thread([self, provider, target, wantFamily, darkMode]() {
            ...
            if (getaddrinfo(target.toStdString().c_str(), nullptr, &hints,
                            &res) == 0 &&
                res) {
              addrinfo *match = nullptr;
              for (addrinfo *r = res; r; r = r->ai_next)
                if (r->ai_family == wantFamily) {
                  match = r;
                  break;
                }
              if (!match)
              ...
              if (match) {
                memcpy(&addr, match->ai_addr,
                       match->ai_addrlen < sizeof(addr) ? match->ai_addrlen
                                                        : sizeof(addr));
            ...
            QMetaObject::invokeMethod(
                qApp,
                [self, net, target, addr, resolved, ipv6, darkMode]() {
                  if (!self)
                    return;
                  if (!resolved) {
...
void MainWindow::onElapsedTimer() {
  ...
  if (m_titleBar)
    m_titleBar->setSubtitle(formatDuration(m_elapsed.elapsed()));
}
...
void MainWindow::onWarmupEnd() {
  ...
  QTimer::singleShot(400, this, [this, gen]() {
    ...
    connect(pollAsn, &QTimer::timeout, this,
            [this, gen, pollAsn, dnsSeen, dnsStall]() {
              if (!m_net || !m_tracing || m_warmupGen != gen) {
                pollAsn->stop();
                pollAsn->deleteLater();
                return;
              ...
              // name other than its bare IP, or when no new name has appeared
              // for a few ticks (the remaining hops simply have no PTR record).
              // This never blocks on a result that isn't coming.
              ...
              for (auto &h : snap) {
                if (h.addr.Ipv4.sin_family == AF_UNSPEC)
                  continue;
                ++addressed;
                if (h.getName() != addr_to_wstring(h.addr))
                  ++named;
              }
              bool dnsSettled;
              if (addressed == 0 || named >= addressed)
                dnsSettled = true;
              else if (*dnsSeen < 0) {
                *dnsSeen = named;
                *dnsStall = 0;
                dnsSettled = false;
              } else if (named > *dnsSeen) {
                *dnsSeen = named;
                *dnsStall = 0;
                dnsSettled = false;
              } else
                dnsSettled = (++*dnsStall >= 3); // ~450 ms without a new name

              if ((!m_asnPending.empty() || !dnsSettled) &&
                  m_elapsed.elapsed() < 16000)
                return;
              pollAsn->stop();
              pollAsn->deleteLater();
              m_asnPending.clear();
  ...
  for (int i = 0; i < rows; ++i) {
    ...
    auto setCell = [&](int col, const QString &text, Qt::Alignment align) {
      ...
      if (!item) {
        ...
        item->setText(text); // skip no-op writes
    };
    constexpr auto C = Qt::AlignCenter | Qt::AlignVCenter;

    setCell(ColHop, QString::number(i + 1), C);
    setCell(ColAsn,
            hasAddr ? getCachedASN(ip, h.addr.Ipv6.sin6_family == AF_INET6) : "-", C);
    ...
    if (h.xmit == 0) {
      for (int c = ColLoss; c < ColCount; ++c)
        setCell(c, "-", C);
    } else {
      ...
      if (auto *lossItem = m_table->item(i, ColLoss)) {
        ...
        lossItem->setData(Qt::ToolTipRole,
                          tip.isEmpty() ? QVariant() : QVariant(tip));
      }
      ...
      setCell(ColJttr, h.returned < 2 ? "-" : QString::number(h.getJitter()),
              C);
    }
  }
...
QString MainWindow::buildJsonExport() const {
  static const QStringList keys = {"hop",  "asn",  "hostname", "ip",
                                   "loss", "sent", "recv",     "best",
                                   "avrg", "wrst", "last",     "jttr"};
  QJsonArray hops;
  ...
  root["test_started"] =
      (m_testStartTime.isValid() ? m_testStartTime
                                 : QDateTime::currentDateTime())
          .toString(Qt::ISODate);
  root["duration_seconds"] =
      static_cast<qint64>(currentTestDurationMs() / 1000);
...
QString MainWindow::buildTextExport() const {
  ...
  for (int c = 0; c < NCOLS; ++c) {
    ...
    for (int i = 0; i < m_table->rowCount(); ++i) {
      ...
      const int len = static_cast<int>(
          (item ? item->text() : QStringLiteral("-")).length());
      if (len > w)
        w = len;
    }
    W[c] = w;
  ...
  out += QString("Date    : %1\n")
             .arg((m_testStartTime.isValid() ? m_testStartTime
                                             : QDateTime::currentDateTime())
  ...
  if (m_net) {
    ...
    for (int i = 0; i < static_cast<int>(st.size()); ++i)
      if (st[i].altCount > 0)
        notes +=
            QString("  Hop %1: replies also arrived from %2 (%3 time(s)) "
                    "\u2014 route change or per-packet load balancing\n")
                .arg(i + 1)
                .arg(QString::fromStdWString(addr_to_wstring(st[i].altAddr)))
                .arg(st[i].altCount);
    for (int i = 0; i < static_cast<int>(st.size()); ++i)
      if (st[i].anomalyCount > 0)
        notes += QString("  Hop %1: %2 probe(s) ended with unexpected ICMP "
                         "status/error %3\n")
                     .arg(i + 1)
                     .arg(st[i].anomalyCount)
                     .arg(st[i].anomalyLast);
    if (!notes.isEmpty())
      out += "\nNotes:\n" + notes;
  }
  return out;
...
void MainWindow::onExport() {
  if (!m_exportBtn->isEnabled())
    return;
  QString target = m_targetEdit->text().trimmed();
  QString stamp = QDateTime::currentDateTime().toString("yyyyMMdd_HHmmss");
  QString defaultName =
      QString("OpenMTR_%1_%2").arg(target.isEmpty() ? "export" : target, stamp);
