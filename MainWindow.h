...
inline QColor ovSystemAccentShade(bool darkMode) {
  const QColor fallback =
      darkMode ? QColor(0x4C, 0xC2, 0xFF) : QColor(0x00, 0x67, 0xC0);
#ifdef Q_OS_WIN
  BYTE palette[32] = {};
...
inline QColor ovAccentSlot(int byteOffset, const QColor &fallback) {
  ...
  const LONG rc = ::RegGetValueW(
      ...
      L"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Accent",
      L"AccentPalette", RRF_RT_REG_BINARY, nullptr, palette, &size);
  if (rc != ERROR_SUCCESS || size < sizeof(palette) || byteOffset < 0 ||
      byteOffset + 2 >= static_cast<int>(sizeof(palette)))
    return fallback;
  return QColor(palette[byteOffset], palette[byteOffset + 1],
                palette[byteOffset + 2]);
#else
  Q_UNUSED(byteOffset);
  return fallback;
#endif
...
struct OvAccentTextShades {
  QColor primary, secondary, tertiary;
};
...
public:
  ...
  static constexpr double kRadius = 5.5;
...
public:
  explicit FluentProgressRing(QWidget *parent = nullptr) : QWidget(parent) {
  ...
  void paintEvent(QPaintEvent *) override {
    ...
    auto bez = [](double x) {
      auto comp = [](double u) { // control points 0.167 / 0.833
        return 3 * u * (1 - u) * (1 - u) * 0.167 + 3 * u * u * (1 - u) * 0.833 +
               u * u * u;
      };
      double lo = 0.0, hi = 1.0;
      for (int i = 0; i < 24; ++i) {
        const double mid = (lo + hi) / 2.0;
        (comp(mid) < x ? lo : hi) = mid;
      }
      return comp((lo + hi) /
                  2.0); // control points are symmetric: y(u) == x(u) form
    };

    const double t =
        double(m_clock.isValid() ? m_clock.elapsed() % 2000 : 0) / 2000.0;
    const double rot = 900.0 * bez(t);
    double startFrac, sweepFrac; // fractions of the circle
    if (t < 0.5) {
      startFrac = 0.0;
      sweepFrac = 0.5 * bez(t * 2.0);
    } else {
      startFrac = 0.5 * bez(t * 2.0 - 1.0);
      sweepFrac = 0.5 - startFrac;
    }
...
public:
  ...
  int pixelMetric(PixelMetric metric, const QStyleOption *option = nullptr,
    ...
    if (widget && widget->property("ovFocusRing").toBool()) {
      if (metric == PM_FocusFrameHMargin)
        return FocusRing::kHMargin;
      if (metric == PM_FocusFrameVMargin)
        return FocusRing::kVMargin;
    }
  ...
  int styleHint(StyleHint hint, const QStyleOption *option = nullptr,
    ...
    if (hint == SH_FocusFrame_Mask && widget &&
        widget->property("ovFocusRing").toBool())
      return 0;
...
public:
  ...
  void setBar(int wrapWidth, int wrapHeight, int thickness, qreal radius,
              const QColor &color) {
...
protected:
  void paintEvent(QPaintEvent *) override {
    ...
    QPainterPath path;
    path.addRoundedRect(
        QRectF(0, -(m_wrapHeight - m_thickness), m_wrapWidth, m_wrapHeight),
        m_radius, m_radius);
...
public:
  Win11Tooltip() : QWidget(nullptr, Qt::ToolTip | Qt::FramelessWindowHint) {
  ...
  void popup(const QString &text, const QPoint &cursorPos, bool dark,
    ...
    m_label->setStyleSheet(
        QStringLiteral("QLabel{background:transparent;font-family:'Segoe "
                       "UI';font-size:12px;color:%1;}")
            .arg(dark ? QStringLiteral("#FFFFFF") : QStringLiteral("#1A1A1A")));
    adjustSize();

    const int gap = 20;
    const QRect scr =
        QApplication::screenAt(cursorPos)
...
protected:
  void paintEvent(QPaintEvent *) override {
    ...
    const QColor fill =
        m_dark ? QColor(0x2C, 0x2C, 0x2C, 244) : QColor(0xF9, 0xF9, 0xF9, 244);
    const QColor border =
        m_dark ? QColor(255, 255, 255, 22) : QColor(0, 0, 0, 10);
...
private:
  void applyChrome() {
    ...
    DwmSetWindowAttribute(hwnd, DWMWA_WINDOW_CORNER_PREFERENCE, &corner,
                          sizeof(corner));
    const COLORREF borderColor = m_dark ? 0x003A3A3A : 0xFFFFFFFE;
...
inline QString captionLabelFromSystem(HWND hwnd, UINT sc,
  ...
  const int len = ::GetMenuStringW(
      sysmenu, sc, buf, static_cast<int>(std::size(buf)), MF_BYCOMMAND);
  if (len <= 0)
    return fallback;
...
public:
  ...
  void setTip(UINT sc, const QString &fallback) {
    m_tipSC = sc;
    m_tipFallback = fallback;
  }
...
protected:
  void paintEvent(QPaintEvent *) override {
    ...
    if (m_kind == CaptionButtonKind::Close) {
      if (m_pressed)
        bg = QColor(0xC4, 0x2B, 0x1C, 0xE6);
      else if (m_hovered)
        bg = QColor(0xC4, 0x2B, 0x1C);
    } else {
      if (m_pressed)
    ...
    if (m_kind == CaptionButtonKind::Close && m_hovered) {
      ...
      const int a = m_dark ? (m_pressed ? 200 : 255) : (m_pressed ? 155 : 228);
  ...
  QChar glyphChar() const {
    ...
    case CaptionButtonKind::Maximize:
      return QChar(m_maximized ? 0xE923 : 0xE922);
...
public:
  ...
  void setMaximized(bool maximized) { m_maxBtn->setMaximized(maximized); }
...
public:
  ...
  void paint(QPainter *painter, const QStyleOptionViewItem &option,
    ...
    const bool cellHovered =
        !dragging && !spacer && option.state.testFlag(QStyle::State_MouseOver);
    ...
    if (rowHovered) {
      const QColor rowFill =
          dark ? QColor(255, 255, 255, 15) : QColor(0, 0, 0, 9);
      painter->fillRect(option.rect, rowFill);
    }
    if (cellHovered) {
      const QColor cellFill =
          dark ? QColor(255, 255, 255, 15) : QColor(0, 0, 0, 9);
    ...
    if (index.column() == ColLoss) {
      ...
      if (ok || dash) {
        ...
        const QColor track = dark ? QColor(0xff, 0xff, 0xff, 0x33)
                                  : QColor(0x00, 0x00, 0x00, 0x1f);
        const QColor trackHov = dark ? QColor(0xff, 0xff, 0xff, 0x55)
        ...
        const int numCellW =
            option.fontMetrics.horizontalAdvance(QStringLiteral("100"));
        const int available = r.width() - 2 * inset;
        const int trackW =
            qMax(0, qMin(trackWIdeal, available - gap - numCellW));
        ...
        painter->drawText(QRect(x + (trackW > 0 ? trackW + gap : 0), r.top(),
                                numCellW, r.height()),
                          Qt::AlignCenter, s);
        painter->restore();
        return;
    ...
    if (!text.isEmpty()) {
      ...
      if (index.column() == ColSent || index.column() == ColRecv) {
        const QModelIndex lossIdx =
            index.model() ? index.model()->index(index.row(), ColLoss)
                          : QModelIndex();
        bool lossOk = false;
        const QString lossText = lossIdx.isValid()
                                     ? lossIdx.data(Qt::DisplayRole).toString()
                                     : QString();
      ...
      if ((index.column() == ColHostname || index.column() == ColIp) &&
        ...
        const QRect iconRect(textRect.right() - 11, textRect.top(), 12,
                             textRect.height());
        painter->save();
...
protected:
  void paintSection(QPainter *painter, const QRect &rect,
    ...
    const QColor border =
        m_dark ? QColor(255, 255, 255, 20) : QColor(0, 0, 0, 20);
    const QColor txt =
        m_dark ? QColor(255, 255, 255, 153) : QColor(0, 0, 0, 140);
    const QColor pillBg =
        m_dark ? QColor(255, 255, 255, 26) : QColor(0, 0, 0, 18);
    const QColor pillTxt =
        m_dark ? QColor(255, 255, 255, 102) : QColor(0, 0, 0, 115);
...
protected:
  bool eventFilter(QObject *o, QEvent *e) override {
    if (o == m_area->viewport()) {
      ...
      case QEvent::MouseMove: {
        ...
        const int widgetLeftInViewport =
            m_area->viewport()->mapFromParent(QPoint(geometry().left(), 0)).x();
        const bool inHotZone = scrollable() && p.x() >= widgetLeftInViewport;
        reveal();
        if (inHotZone) {
          const bool justEntered =
      ...
      case QEvent::Leave:
        ...
        if (!m_dragging) {
          ...
          const bool overUs =
              geometry().contains(m_area->mapFromGlobal(globalPos));
          if (!overUs) {
            setHovered(false);
            setAttribute(Qt::WA_TransparentForMouseEvents, true);
          }
  ...
  void mousePressEvent(QMouseEvent *e) override {
    if (e->button() != Qt::LeftButton || !scrollable()) {
      e->ignore();
      return;
    }
  ...
  void mouseMoveEvent(QMouseEvent *e) override {
    ...
    if (span <= 0)
      return;
    const int top =
        qBound(tr.top(), e->pos().y() - m_grabDelta, tr.top() + span);
    QScrollBar *sb = m_area->verticalScrollBar();
  ...
  void mouseReleaseEvent(QMouseEvent *) override {
    ...
    if (!m_hovered)
      m_hideTimer.start();
    update();
    if (QWidget *vp = m_area->viewport())
      vp->update();
  }

  void paintEvent(QPaintEvent *) override {
    if (m_opacity <= 0.01 || !scrollable())
      return;
    ...
    if (m_expandFrac > 0.01) {
      ...
      QRectF trackF(thumbCX - curTrackW / 2.0, tr.top(), curTrackW,
                    tr.height());
      p.setBrush(track);
      p.setPen(Qt::NoPen);
    ...
    if (m_expandFrac > 0.01) {
      const QColor arrow =
          m_dark ? QColor(255, 255, 255, qRound(qreal(a) * m_expandFrac))
...
private:
  ...
  QRect thumbTrackRect() const {
    const QRect tr = trackRect();
    return QRect(tr.left(), tr.top() + kArrowRes, tr.width(),
                 tr.height() - 2 * kArrowRes);
  }
  ...
  QRect thumbRect() const {
    ...
    int thumbH =
        total > 0 ? qRound(double(ttr.height()) * page / total) : ttr.height();
    thumbH = qBound(kMinThumb, thumbH, ttr.height());
    const int span = ttr.height() - thumbH;
    const int y =
        ttr.top() +
        (span > 0 ? qRound(double(span) * (sb->value() - sb->minimum()) / range)
                  : 0);
...
private:
  ...
  void updateInputStyle(QWidget *input, InputAccentBar *accent, bool active,
...
public:
  static void show(QWidget *parent, const QString &title,
    ...
    MicaDialog dlg(parent, title, message, darkMode, linkUrl, linkText,
                   linkUrl2, linkText2);
    dlg.exec();
...
private:
  explicit MicaDialog(QWidget *parent, const QString &title,
    ...
    if (!linkUrl.isEmpty()) {
      m_linkBtn =
          new QPushButton(linkText.isEmpty() ? linkUrl : linkText, m_card);
      ...
      if (!linkUrl2.isEmpty()) {
        m_linkBtn2 =
            new QPushButton(linkText2.isEmpty() ? linkUrl2 : linkText2, m_card);
        ...
        connect(linkBtn2, &QPushButton::clicked, this,
                [url2]() { QDesktopServices::openUrl(QUrl(url2)); });
    ...
    closeBtn->setStyleSheet(
        darkMode
            ? "QPushButton { background-color: rgba(255,255,255,0.06); color: "
              "#ffffff;"
              " border: 1px solid rgba(255,255,255,0.08); border-top-color: "
              "rgba(255,255,255,0.15);"
              " border-radius: 4px; padding: 5px 12px; outline: none; }"
              " QPushButton:hover { background-color: rgba(255,255,255,0.08);"
              " border-color: rgba(255,255,255,0.09); border-top-color: "
              "rgba(255,255,255,0.15); }"
              " QPushButton:pressed { background-color: rgba(255,255,255,0.03);"
              " border-color: rgba(255,255,255,0.06); color: "
              "rgba(255,255,255,0.70); }"
            : "QPushButton { background-color: rgba(255,255,255,0.85); color: "
              "rgba(0,0,0,0.89);"
              " border: 1px solid rgba(0,0,0,0.06); border-bottom-color: "
              "rgba(0,0,0,0.16);"
              " border-radius: 4px; padding: 5px 12px; outline: none; }"
              " QPushButton:hover { background-color: rgba(243,243,243,0.95);"
              " border-color: rgba(0,0,0,0.08); border-bottom-color: "
              "rgba(0,0,0,0.16); }"
    ...
    auto *sep = new SepWidget(darkMode ? QColor(255, 255, 255, 20)
    ...
    auto *footerWidget = new FooterWidget(
        darkMode ? QColor(0x14, 0x14, 0x14) : QColor(0xF3, 0xF3, 0xF3), this);
    ...
    m_focusRing->setRingColor(darkMode ? QColor(255, 255, 255)
                                       : QColor(0, 0, 0, 230));
    m_focusRing->hide();
