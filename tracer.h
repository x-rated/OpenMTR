...
struct HopRecord {
  ...
  unsigned long total;
  unsigned long
      jitterSum; // sum of |RTT - previous RTT| over consecutive replies
  int last;
  int best;
  int worst;
  char name[256];
...
public:
  ...
  // had its fair chance).
  int GetLastAlive() const {
    return m_lastAlive.load(std::memory_order_relaxed);
  }
  bool IsParkingEnabled() const {
    return m_parkingEnabled.load(std::memory_order_relaxed);
  }
