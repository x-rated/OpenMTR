...
// UI dependency.
OpenMTRNet::OpenMTRNet(const OpenMTROptions &options) : opts(options) {
...
void OpenMTRNet::DoTrace(sockaddr *dest) {
  ...
  for (int i = 0; i < MAX_HOPS; ++i) {
    ...
    if (hs.hIcmp == INVALID_HANDLE_VALUE) {
      hs.retired = true;
      continue;
    }
  ...
  auto complete = [&](HopState &hs) {
    ...
    if (status != IP_SUCCESS && status != IP_TTL_EXPIRED_TRANSIT) {
      ...
      RecordProbe(hs.ttl - 1, false, 0,
                  status == IP_REQ_TIMED_OUT ? 0ul : status);
    }
    hs.pending = false;
  ...
  auto submit = [&](HopState &hs) {
    DWORD ret =
        isV6 ? Icmp6SendEcho2(hs.hIcmp, hs.hEvent, nullptr, nullptr,
                              const_cast<sockaddr_in6 *>(&srcAny), &destAddr6,
                              hs.requestData.data(), payloadLen, &hs.ipOptions,
                              hs.replyBuffer.data(),
                              (DWORD)hs.replyBuffer.size(), ECHO_REPLY_TIMEOUT)
             : IcmpSendEcho2(hs.hIcmp, hs.hEvent, nullptr, nullptr, destAddr4,
                             hs.requestData.data(), payloadLen, &hs.ipOptions,
                             hs.replyBuffer.data(),
                             (DWORD)hs.replyBuffer.size(), ECHO_REPLY_TIMEOUT);
  ...
  while (tracing && anyActive) {
    ...
    int count = 0;
    handles[count] = m_stopEvent;
    handleHop[count] = -1;
    ++count;
    for (auto &hs : hops) {
      if (hs.pending) {
        handles[count] = hs.hEvent;
        handleHop[count] = hs.ttl - 1;
        ++count;
      }
    }
...
void OpenMTRNet::DoTrace(sockaddr *dest) {
  ...
  auto submit = [&](PosixHopState &hs) {
    ...
    if (isV6) {
      ...
      int sent = ::sendto(fd, pkt.data(), pkt.size(), 0, (sockaddr*)&destAddr6, sizeof(sockaddr_in6));
      if (sent < 0) {
      ...
      int sent = ::sendto(fd, pkt.data(), pkt.size(), 0, (sockaddr*)&destAddr4, sizeof(sockaddr_in));
      if (sent < 0) {
        SetErrorName(hs.ttl - 1, IP_GENERAL_FAILURE);
        RecordProbe(hs.ttl - 1, false, 0, IP_GENERAL_FAILURE);
        hs.pending = false;
  ...
  while (tracing && anyActive) {
    ...
    if (fds[0].revents & POLLIN) {
      char dummy;
      (void)::read(m_stopPipe[0], &dummy, 1);
      break;
    }
          ...
          if (icmp->icmp_type == ICMP_ECHOREPLY) {
            ...
            if (icmp_len >= 8 + 20 + 8) {
              ...
              if (inner_ip_hdr_len >= 20 && icmp_len >= 8 + inner_ip_hdr_len + 8) {
                struct icmp *inner_icmp = (struct icmp *)(readBuf + ip_hdr_len + 8 + inner_ip_hdr_len);
...
static void DnsResolverThread(void *p) {
  ...
  if (!getnameinfo(sa, salen, hostname, NI_MAXHOST, nullptr, 0,
                   NI_NUMERICHOST)) {
...
int OpenMTRNet::GetMax() { return m_maxHops.load(std::memory_order_relaxed); }
...
int OpenMTRNet::RecalcMaxLocked() {
  ...
  if (m_hops[0].addr6.sin6_family == AF_INET6) {
    for (; max < MAX_HOPS && memcmp(&m_hops[max++].addr6.sin6_addr,
                                    &last_remote_addr6, sizeof(in6_addr));)
      ;
    if (max == MAX_HOPS) {
      while (max > 1 &&
             !memcmp(&m_hops[max - 1].addr6.sin6_addr,
                     &m_hops[max - 2].addr6.sin6_addr, sizeof(in6_addr)) &&
#ifdef _WIN32
             (m_hops[max - 1].addr6.sin6_addr.u.Word[0] |
              m_hops[max - 1].addr6.sin6_addr.u.Word[1] |
              m_hops[max - 1].addr6.sin6_addr.u.Word[2] |
              m_hops[max - 1].addr6.sin6_addr.u.Word[3] |
              m_hops[max - 1].addr6.sin6_addr.u.Word[4] |
              m_hops[max - 1].addr6.sin6_addr.u.Word[5] |
              m_hops[max - 1].addr6.sin6_addr.u.Word[6] |
              m_hops[max - 1].addr6.sin6_addr.u.Word[7])
#else
             ([](const in6_addr& a) {
               for (int k = 0; k < 16; ++k) if (a.s6_addr[k] != 0) return true;
               return false;
    ...
    for (; max < MAX_HOPS &&
           m_hops[max++].addr.sin_addr.s_addr != last_remote_addr.s_addr;)
      ;
    if (max == MAX_HOPS) {
      while (max > 1 &&
             m_hops[max - 1].addr.sin_addr.s_addr ==
                 m_hops[max - 2].addr.sin_addr.s_addr &&
             m_hops[max - 1].addr.sin_addr.s_addr)
        --max;
    }
  }
...
void OpenMTRNet::SetErrorName(int at, DWORD errnum) {
  ...
  case IP_NO_RESOURCES:
    name = "Insufficient IP resources.";
    break;
  case IP_BAD_OPTION:
    name = "Bad IP option.";
    break;
  ...
  case IP_TTL_EXPIRED_REASSEM:
    name = "TTL expired during reassembly.";
    break;
...
void OpenMTRNet::RecordProbe(int at, bool replied, int rtt,
  ...
  if (replied) {
    ...
    if (m_hops[at].returned > 0)
      m_hops[at].jitterSum += (rtt > m_hops[at].last)
                                  ? (unsigned long)(rtt - m_hops[at].last)
                                  : (unsigned long)(m_hops[at].last - rtt);
    m_hops[at].last = rtt;
    m_hops[at].total += rtt;
    if (m_hops[at].returned == 0 || m_hops[at].best > rtt)
      m_hops[at].best = rtt;
    if (m_hops[at].worst < rtt)
      m_hops[at].worst = rtt;
    ++m_hops[at].returned;
